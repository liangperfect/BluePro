...
public class ChartView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d("chenliang", "onMeasure is widthMewasureSpec" + widthMeasureSpec +
                "  heightMeasureSpec" + heightMeasureSpec);
    }

    @Override
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        Log.d("chenliang", "width->" + width + "  height->" + height);
        if (changed) {
            ...
            Log.d("chenliang", "width->" + width + "  height->" + height);
            //Y轴文本最大宽度
            float textYWdith = getTextBounds("000", xyTextPaint).width();//文本在Y轴上显示的最小宽度
...
//            //X轴文本最大高度
            ...
            Log.d("chenliang", "minXInit->" + minXInit);
            maxXInit = xInit;
            Log.d("chenliang", "maxXInit->" + maxXInit);
        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (interval * xValue.size() > width - xOri) {//当期的宽度不足以呈现全部数据
                    ...
                    startX = event.getX();
                    Log.d("chenliang", "dis->" + dis + "  startX->" + startX);
